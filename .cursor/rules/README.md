...
8. **Проект развернут на отдельном Linux сервере** - не запускать Docker или команды локально
9. **Никогда не выполнять команды без разрешения пользователя** - особенно DevOps/Server команды
10. **Полуавтоматический режим** - предлагать, не реализовывать без утверждения
