...
9. **КРИТИЧЕСКИ ВАЖНО**: Пользователь работает на Windows (PowerShell), подключается к Linux-серверу напрямую через SSH-клиент (PuTTY, MobaXterm, etc.), НЕ использует CI/CD (GitHub Actions, GitLab CI) или панели управления
10. **ВСЕ команды должны быть для SSH/Bash на Linux-сервере, предлагаемые пользователю для копирования и выполнения вручную**
11. **Никогда не выполнять команды без разрешения пользователя** - особенно DevOps/Server команды
12. **Полуавтоматический режим** - предлагать, не реализовывать без утверждения
