#!/bin/bash
# sync_db_password.sh - Синхронизация пароля БД между secrets и PostgreSQL
...
echo ""
log_info "=== Синхронизация пароля базы данных PostgreSQL ==="
echo ""
...
# Попытка подключения с паролем из secrets
log_info "Проверка подключения с паролем из secrets..."
export PGPASSWORD="$DB_PASSWORD"
if $COMPOSE_CMD exec -T postgres psql -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1;" >/dev/null 2>&1; then
    log_success "Пароль БД уже правильный, синхронизация не требуется"
    exit 0
fi

log_warning "Подключение с паролем из secrets не удалось, обновляем пароль в БД..."

# Пробуем подключиться с дефолтным паролем
...
if $COMPOSE_CMD exec -T postgres psql -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1;" >/dev/null 2>&1; then
    ...
    echo ""
    log_info "Рекомендуется перезапустить контейнер php-fpm для применения изменений:"
    log_info "  $COMPOSE_CMD restart php-fpm"
    exit 0
else
