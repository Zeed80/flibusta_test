# Как запустить тесты OPDS

## 🔍 Шаг 1: Найдите правильное имя контейнера PHP-FPM

Выполните на сервере (через SSH):

```bash
# Вариант 1: Показать все контейнеры
docker ps

# Вариант 2: Найти контейнер PHP-FPM
docker ps | grep php-fpm

# Вариант 3: Использовать скрипт поиска
chmod +x /flibusta_test/scripts/find_php_fpm_container.sh
/flibusta_test/scripts/find_php_fpm_container.sh
```

**Ожидаемый результат:**
```
CONTAINER ID   IMAGE                    STATUS
abc123def456   flibusta_test-php-fpm    Up 2 hours ago
```

Имя контейнера будет что-то вроде:
- `flibusta_test-php-fpm-1`
- `flibusta_php-fpm_1`
- `php-fpm-1`
- или другое, в зависимости от вашей конфигурации

## 🚀 Шаг 2: Запустите тесты

После того, как вы нашли правильное имя контейнера, выполните:

```bash
# Замените CONTAINER_NAME на найденное имя
docker exec -it CONTAINER_NAME php /application/tests/test_opds.php
```

**Примеры:**

```bash
# Если контейнер называется flibusta_test-php-fpm-1
docker exec -it flibusta_test-php-fpm-1 php /application/tests/test_opds.php

# Если контейнер называется flibusta_php-fpm_1
docker exec -it flibusta_php-fpm_1 php /application/tests/test_opds.php

# Автоматический поиск (если контейнер единственный с php-fpm)
docker exec -it $(docker ps | grep php-fpm | awk '{print $1}') php /application/tests/test_opds.php
```

## 🔧 Альтернативный способ: через docker-compose

Если у вас есть доступ к `docker-compose.yml`:

```bash
# Перейдите в директорию проекта
cd /flibusta_test

# Запустите тесты через docker-compose
docker-compose exec php-fpm php /application/tests/test_opds.php
```

## 📊 Ожидаемый результат

После успешного запуска вы увидите:

```
╔════════════════════════════════════════╗
║     OPDS ТЕСТОВАНИЕ СИСТЕМЫ          ║
╚════════════════════════════════════════╝
Базовый URL: http://localhost:27100/opds/
Запуск: 2024-12-26 18:00:00

========================================
ТЕСТ: Главная страница OPDS (/)
Результат: ✅ PASSED
Сообщение: Все проверки пройдены
...

========================================
СВОДНАЯ СТАТИСТИКА ТЕСТОВ
Всего тестов: 25
Пройдено: 23 (92%)
Провалено: 2 (8%)
Успешность: 92%
```

## ❌ Если тесты не запускаются

### Проблема 1: "No such container"
**Решение:** Используйте команду из Шага 1 для поиска правильного имени контейнера.

### Проблема 2: "Container is not running"
**Решение:**
```bash
# Запустите контейнеры
docker-compose up -d

# Проверьте статус
docker-compose ps
```

### Проблема 3: "Permission denied"
**Решение:**
```bash
# Проверьте права на файл теста
docker exec -it CONTAINER_NAME ls -la /application/tests/test_opds.php

# Если нужно, установите права
docker exec -it CONTAINER_NAME chmod +x /application/tests/test_opds.php
```

### Проблема 4: "File not found"
**Решение:**
```bash
# Проверьте, что файл существует
docker exec -it CONTAINER_NAME ls -la /application/tests/

# Если файла нет, проверьте, что изменения были загружены на сервер
```

## 🎯 Быстрая команда (копируйте и вставляйте)

```bash
# Автоматический поиск и запуск тестов
CONTAINER=$(docker ps | grep php-fpm | awk '{print $NF}' | head -1)
if [ -n "$CONTAINER" ]; then
    echo "Найден контейнер: $CONTAINER"
    docker exec -it "$CONTAINER" php /application/tests/test_opds.php
else
    echo "Контейнер PHP-FPM не найден!"
    echo "Запустите: docker ps | grep php-fpm"
fi
```

## 📝 Примечания

- Тесты требуют, чтобы контейнеры были запущены (`docker-compose up -d`)
- Тесты проверяют доступность OPDS через HTTP, поэтому нужен работающий веб-сервер
- Если тесты показывают ошибки, проверьте логи: `docker-compose logs php-fpm --tail=50`
