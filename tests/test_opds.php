 ...
 * Запускается: php tests/test_opds.php
 * 
 * Тестирует:
 * - Все OPDS endpoint-ы
...
// Базовый URL OPDS
$baseUrl = 'http://localhost:27100/opds';
$tests = [];
$results = [];
...
function testNewBooks() {
    ...
    $response = httpGet($baseUrl . '/list/', $testName);
...
function testSearchBooks() {
    ...
    $response = httpGet($baseUrl . '/search?q=пушкин', $testName);
...
function testSearchAuthors() {
    ...
    $response = httpGet($baseUrl . '/search?by=author&q=пушкин', $testName);
...
function testGenres() {
    ...
    $response = httpGet($baseUrl . '/genres', $testName);
...
function testAuthorsIndex() {
    ...
    $response = httpGet($baseUrl . '/authorsindex', $testName);
...
function testSequencesIndex() {
    ...
    $response = httpGet($baseUrl . '/sequencesindex', $testName);
...
function testBookshelves() {
    ...
    $response = httpGet($baseUrl . '/favs', $testName);
...
function testPagination() {
    ...
    $response = httpGet($baseUrl . '/list/?page=2', $testName);
...
function testOPDSVersion() {
    ...
    $response = httpGet($baseUrl . '/?opds_version=1.0', $testName);
...
function testOPDS12Version() {
    ...
    // Проверяем наличие OPDS 1.2 специфичных элементов
    $hasFacetGroup = strpos($response['content'], 'opds:facetGroup') !== false;
    if (!$hasFacetGroup) {
        // FacetGroup может отсутствовать в главном фиде
        // Это нормально
    }
    
    testResult($testName, true, "Автоматическое определение OPDS 1.2 работает");
}
...
function testETagAndCaching() {
    ...
    $response1 = httpGet($baseUrl, $testName);
    $response2 = httpGet($baseUrl, $testName);
    ...
    // Проверяем наличие ETag заголовка
    $headers1 = curl_getinfo(curl_init());
...
function testEmptySearchQuery() {
    ...
    $response = httpGet($baseUrl . '/search?q=', $testName);
